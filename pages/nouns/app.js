...
const PROG_KEY = 'gr_nouns_progress';
...
function renderBatchOptions() {
  ...
  BATCHES.forEach(batch => {
    ...
    ol.value = batch.batchId;
    ol.textContent = `${batch.batchHeader} (${cnt})`;
    learnSelect.appendChild(ol);
    const oq = document.createElement('option');
    oq.value = batch.batchId;
    oq.textContent = `${batch.batchHeader} (${cnt})`;
    quizSelect.appendChild(oq);
  });

  if (Array.from(learnSelect.options).some(o => o.value === curLearn)) learnSelect.value = curLearn;
  else learnSelect.value = '-1';
  if (Array.from(quizSelect.options).some(o => o.value === curQuiz)) quizSelect.value = curQuiz;
  else quizSelect.value = '-2';
}
...
function renderPlanTable() {
  ...
  tbody.innerHTML = BATCHES.map((batch, idx) => {
    ...
    const cnt = visibleWords.filter(w => w.batch === bId).length;
    if (cnt === 0) return '';
    return `<tr class="plan-row" onclick="goToBatch(${batch.batchId})" title="Открыть карточки: ${batch.batchName}">
  ...
  document.getElementById('plan-title').textContent = `Обзор: ${total} существительных`;
...
function prevCard() { if (learnIdx <= 0) return; learnIdx--; showLearnCard(); }
function nextCard() { if (learnIdx >= learnQueue.length) return; learnIdx++; showLearnCard(); }
...
function startQuiz() {
  ...
  if (pool.length < 2) { alert('Недостаточно слов для теста. Добавьте больше слов или измените фильтры.'); return; }
...
function buildFormHtml(w) {
  const rows = [];
  if (w.male && w.male !== '-') rows.push(`<div class="quiz-q-form-row"><span class="lbl lbl-m">муж</span>${escapeHtml(w.male)}</div>`);
  if (w.female && w.female !== '-') rows.push(`<div class="quiz-q-form-row"><span class="lbl lbl-f">жен</span>${escapeHtml(w.female)}</div>`);
  if (w.neuter && w.neuter !== '-') rows.push(`<div class="quiz-q-form-row"><span class="lbl lbl-n">ср</span>${escapeHtml(w.neuter)}</div>`);
  return rows.join('');
}
...
function nextQuizQuestion() {
  ...
  const visibleWords = getVisibleWords();
  const wrongPool = visibleWords.filter(w => w !== correct);
  ...
  if (mode === 'gr-ru') {
    ...
    // ru-gr
    modeLabel.textContent = 'Выбери греческое существительное:';
...
function checkAnswer(btn, chosenKey, correctKey) {
  ...
  if (isCorrect) { btn.classList.add('correct'); quizCorrectCount++; }
    ...
    const correctBtn = document.querySelector(`.quiz-option[data-key="${correctKey}"]`);
  ...
  for (let b = 0; b < BATCHES.length; b++) {
    ...
    const wordRows = bWords.map(w => {
      ...
      return `<div class="word-status-row">
        ...
        <span class="word-status-main">${escapeHtml(getWordKey(w))}</span>
...
function resetBatchProgress(batchId) {
  ...
  const visibleWords = getVisibleWords();
  WORDS.forEach((w, i) => {
    if (w.batch === batchId && visibleWords.includes(w)) {
      delete progress[i];
      quizStreaks[i] = 0;
    }
  });
...
function resetAll() {
  if (confirm('Сбросить весь прогресс по существительным?')) {
...
resetQuizUiState();
  ...
  .catch(err => console.error('Failed to load nouns-data.json:', err));
